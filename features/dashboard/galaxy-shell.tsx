...
export function GalaxyShell({ children, compact = false }: { children: React.ReactNode; compact?: boolean }) {
  ...
  return (
    <div className="min-h-screen bg-white">
            ...
            <>
                  ...
                  <div key={section.title || "root"} className="mb-5">
                    ...
                    <div className="space-y-1">
                      {section.items.map(([label, Icon, badge]) => (
                        <div key={label} className="flex h-[29px] items-center gap-3 rounded-md px-1.5 text-[13px] text-gray-800 hover:bg-white">
                    ...
                    {"note" in section && <p className="mt-3 px-2 text-[11px] leading-4 text-gray-500">{section.note}</p>}
                  </div>
                ))}
              </nav>
              <div className="border-t border-gray-200 p-2">
                ...
                <div className="flex items-center gap-2 px-3 py-2 text-xs font-medium">
                  <UserRound size={22} className="rounded-full bg-gray-200 p-1 text-gray-600" />
                  Pranav Abhimanyu
                </div>
              </div>
            </>
      ...
      <main className={cn("min-h-screen pt-[50px] transition-[padding] duration-200", hidden ? "pl-0" : "pl-[213px] max-sm:pl-0")}>{children}</main>
...
function SettingsDialog({ onClose }: { onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 grid place-items-center bg-black/75 px-4">
      <section className="grid h-[500px] w-full max-w-[746px] grid-cols-[200px_1fr] overflow-hidden rounded-[13px] border border-gray-200 bg-white shadow-float max-sm:h-[calc(100vh-92px)] max-sm:grid-cols-1">
        <aside className="border-r border-gray-200 bg-[#fbfbfc] p-3 max-sm:hidden">
          <button className="mb-5 grid h-8 w-8 place-items-center rounded-md hover:bg-white" onClick={onClose} aria-label="Close">
            ×
          </button>
          <div className="space-y-1">
            {tabs.map(([label, Icon], index) => (
              <button key={label} className={cn("flex h-[34px] w-full items-center gap-3 rounded-md px-3 text-[13px]", index === 0 ? "bg-white font-medium shadow-card" : "text-gray-600 hover:bg-white")}>
                <Icon size={15} /> {label}
              </button>
            ))}
          </div>
        ...
        <div className="min-w-0">
          <header className="flex h-[50px] items-center border-b border-gray-200 px-5">
            <button className="mr-3 grid h-7 w-7 place-items-center rounded-md hover:bg-gray-100 sm:hidden" onClick={onClose} aria-label="Close">
              ×
            </button>
          ...
          <div className="p-5">
            ...
            <ActionRow title="Create Organization" description="Start collaborating with your team" action="Create" icon={<Building2 size={14} />} onClick={() => setMessage("Organization creation is ready.")} />
            ...
            <ActionRow
              ...
              danger
              icon={<Trash2 size={14} />}
              onClick={() => setMessage("Delete account requested. Connect Clerk user deletion in production.")}
            />
            {message && <div className="mt-4 rounded-md border border-gray-200 bg-[#fbfbfc] px-3 py-2 text-[12px] text-gray-600">{message}</div>}
          </div>
        </div>
      </section>
    </div>
...
function SectionTitle({ children, className }: { children: React.ReactNode; className?: string }) {
  return <div className={cn("mb-3 text-[11px] font-semibold uppercase tracking-[0.04em] text-gray-500", className)}>{children}</div>;
}
...
function ActionRow({ title, description, action, icon, danger, onClick }: { title: string; description: string; action: string; icon: React.ReactNode; danger?: boolean; onClick?: () => void }) {
  return (
    <div className="flex min-h-[56px] items-center border-b border-gray-200 py-2">
      <div>
        <div className={cn("text-[13px] font-medium", danger && "text-red-500")}>{title}</div>
        <div className="mt-1 text-[11px] text-gray-500">{description}</div>
      </div>
      <button className={cn("ml-auto flex h-8 items-center gap-2 rounded-full border px-4 text-[11px]", danger ? "border-red-200 text-red-500" : "border-gray-200")} onClick={onClick}>
        {action} {icon}
      </button>
    </div>
  );
...
function CommandPalette({ onClose }: { onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 grid place-items-center bg-black/75 px-4">
      <section className="w-full max-w-[568px] overflow-hidden rounded-[14px] border border-gray-200 bg-white shadow-float">
        <div className="flex h-[42px] items-center gap-2 border-b border-gray-200 px-4">
          <Search size={16} className="text-gray-400" />
          <input className="min-w-0 flex-1 text-[13px] outline-none placeholder:text-gray-400" placeholder="Search models, tools, features, pages, and more..." autoFocus />
          <span className="rounded bg-gray-100 px-2 py-1 text-[9px] text-gray-400">↑↓ navigate</span>
          <button className="grid h-6 w-6 place-items-center rounded-md text-gray-500 hover:bg-gray-100" onClick={onClose} aria-label="Close">×</button>
        </div>
        <div className="galaxy-scrollbar max-h-[333px] overflow-y-auto p-3">
          ...
          {quick.map(([title, tag, Icon, active]) => (
            <PaletteItem key={title} title={title} tag={tag} icon={<Icon size={19} />} active={active} />
          ))}
          <PaletteLabel className="mt-4">Trending</PaletteLabel>
          {trending.map(([title, tag, Icon]) => (
            <PaletteItem key={title} title={title} tag={tag} icon={<Icon size={19} />} />
          ))}
        </div>
...
function PaletteLabel({ children, className }: { children: React.ReactNode; className?: string }) {
  return (
    <div className={cn("mb-2 flex items-center gap-2 text-[11px] font-semibold uppercase text-gray-500", className)}>
      <FileText size={13} /> {children}
    </div>
  );
}

function PaletteItem({ title, tag, icon, active }: { title: string; tag: string; icon: React.ReactNode; active?: boolean }) {
  return (
    <button className={cn("flex h-[49px] w-full items-center gap-3 rounded-lg px-3 text-left", active ? "bg-[#f1edff] text-galaxy-purple" : "hover:bg-gray-50")}>
      <span className="grid h-[31px] w-[31px] place-items-center rounded-md bg-white text-gray-600 shadow-card">{icon}</span>
      <span className="min-w-0">
        <span className="block text-[13px] font-semibold">
          {title} <span className="ml-2 rounded bg-gray-100 px-1.5 py-0.5 text-[9px] uppercase text-gray-500">{tag}</span>
        </span>
        <span className="mt-1 block truncate text-[11px] text-gray-500">Access workflows, tools, and useful pages</span>
      </span>
